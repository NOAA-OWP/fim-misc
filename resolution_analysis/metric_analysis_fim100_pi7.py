...
def compute_logs(metrics):
    ...
    metrics['median_slope_log'] = np.log10(metrics['median_slope'] + 1e-8)
    metrics['freq_high_dev_log'] = np.log10(metrics['freq_high_dev'] + 1e-8)
...
def covariate_scatter_plots(metrics, plot_output_dir, benchmark_source=None):
    ...
    if (benchmark_source is not None) & (benchmark_source != 'all'):
        metrics = metrics[metrics['collection_id'] == benchmark_source]

    resolutions = metrics['resolution_m']
    magnitudes = metrics['scenario']

    # drop resolution and magnitude columns
    metrics = metrics.drop(columns=['resolution_m', 'scenario'])
    ...
    elif benchmark_source in ['ble', 'mip']:
        mag_vals = ['2yr', '5yr', '10yr', '25yr', '50yr', '100yr']
    else:
        mag_vals = ['100yr', '500yr']

    # reorder the categories
    metrics.loc[:, 'scenario'] = pd.Categorical(magnitudes, categories=mag_vals, ordered=True)
    metrics.loc[:, 'resolution_m'] = pd.Categorical(resolutions, categories=[10, 5, 3], ordered=True)
    ...
    for i, metric in enumerate(['MCC', 'CSI', 'TPR', 'FAR']):
        ...
        sns.scatterplot(x='median_slope_log', y=metric, data=metrics, ax=axs[0, i], hue='scenario', style='resolution_m')
        sns.scatterplot(x='freq_high_dev_log', y=metric, data=metrics, ax=axs[1, i], hue='scenario', style='resolution_m')
...
def box_plots_by_resolution_and_magnitude(covariates, benchmark_source, plot_output_dir):
    ...
    magnitudes = covariates['scenario']

    # drop the magnitude column
    covariates = covariates.drop(columns='scenario')
    ...
    # reorder the categories
    covariates.loc[:, 'scenario'] = pd.Categorical(magnitudes, categories=vals, ordered=True)
    ...
    for i, metric in enumerate(['MCC', 'CSI', 'TPR', 'FAR']):
        ...
        sns.boxplot(
            x='scenario', y=metric, hue='resolution_m', data=covariates, ax=axs[mi], hue_order=[10, 5, 3], palette='bright'
        )
...
def scatter_plot_matrix(metrics, plot_output_dir):
    ...
    variables = [
        'MCC', 'CSI', 'TPR', 'FAR',
        'median_slope_log', 'freq_high_dev_log', 'scenario', 'resolution_m', 'collection_id', 'percent_covered_by_tiles'
    ]
...
def box_plots_by_tile_availability(metrics, plot_output_dir):
    ...
    for i, metric in enumerate(metrics_list):
        ...
        # make a boxplot
        sns.boxplot(
            x='tile_availability', y=metric, data=metrics, ax=ax, palette='bright', order=['none', 'partial', 'full'], hue='resolution_m', hue_order=[10, 5, 3]
        )
...
def box_plot_of_compute_performance_by_resolutions(compute_metrics, plot_output_dir, usd=False, device='s3'):
    '''
    ...
    total_hucs = compute_metrics.HUC12.nunique()

    # create a figure
    fig, axs = plt.subplots(1, 2, figsize=(9, 6))

    all_caps_device = device.upper()
    ...
    if usd:
        ...
        y_storage = f'{device}_storage_cost_month_usd'
        title_storage = f'Monthly {all_caps_device} Storage Cost (USD) by Resolution'
        y_label_storage = f'Monthly {all_caps_device} Storage Cost (USD)'
        median_df_columns = [f'Median {all_caps_device} Compute Cost (USD)', f'Median Monthly {all_caps_device} Storage Cost (USD)']
        total_df_columns = [f'Total {all_caps_device} Compute Cost (USD)', f'Total Monthly {all_caps_device} Storage Cost (USD)']
    else:
    ...
    median_df = pd.concat(
        (
            compute_metrics.groupby("resolution_m")[y_time].median(),
            compute_metrics.groupby("resolution_m")[y_storage].median() 
        ),
        axis=1
    )
    ...
    # make a boxplot
    sns.boxplot(
        x='resolution_m', y=y_time, data=compute_metrics, ax=axs[0], order=[10, 5, 3]
    )
    sns.boxplot(
        x='resolution_m', y=y_storage, data=compute_metrics, ax=axs[1], order=[10, 5, 3]
    )
    ...
    # make figure title
    fig.suptitle('HUC12 Level')

    # save the figure
    units_name = 'USD' if usd else 'mins'
    if usd:
        plt.savefig(os.path.join(plot_output_dir, f'box_plot_of_compute_performance_by_resolutions_{units_name}_{device}.png'))
    else:
...
def build_tile_regression_model(metrics, plot_output_dir, categorical=True):
    ...
    for i, metric in enumerate(metrics_list):
        ...
        ax.text(0.01, 0.01, f'Corr. Coef.: {corr:.4f}\ny={slope:.4f}x+{intercept:.4f}\n99% CI: ({lower_ci:.4f}, {upper_ci:.4f})', fontsize=10, transform=ax.transAxes)
...
def difference_histogram(metrics, plot_output_dir):
    ...
    # only keep stac_item-id that have all resolutions
    metrics = metrics.groupby('stac_item_id').filter(lambda x: set(x['resolution_m']) == {10, 5, 3})

    # make a list of metrics
    metrics_list = ['MCC', 'CSI', 'TPR', 'FAR']

    # subset benchmark source
    #metrics = metrics[metrics['collection_id'].isin(['usgs', 'nws'])]
    ...
    metrics = metrics.drop(columns=['scenario', 'resolution_m'])
    ...
    for diff in ['3-10', '3-5', '5-10']:
        ...
        metrics_diff = (
            metrics[(metrics.resolution_m == reses[0])].set_index(['stac_item_id', 'scenario', 'collection_id'])[metrics_list] - metrics[(metrics.resolution_m == reses[1])].set_index(['stac_item_id', 'scenario', 'collection_id'])[metrics_list]
        ).reset_index().dropna().merge(metrics.drop(columns=metrics_list), on=['stac_item_id', 'scenario', 'collection_id'], how='left').drop_duplicates(subset=['stac_item_id', 'scenario', 'collection_id'] + metrics_list)
    ...
    metrics_diff = pd.concat(metrics_diff_list, ignore_index=True).drop(columns='resolution_m')
    ...
    for diff in ['3-10', '3-5', '5-10']:
        ...
        for i, metric in enumerate(metrics_list):
            ...
            # make a boxplot
            sns.histplot(
                x=metric, data=metrics_diff[metrics_diff['resolution_diff'] == diff], ax=ax, bins=250
            )
            ...
            ax.text(0.68, 0.45, f'Mean: {summary["mean"]:.4f}\nStd: {summary["std"]:.4f}\nMedian: {summary["50%"]:.4f}\n25th: {summary["25%"]:.4f}\n75th: {summary["75%"]:.4f}\nMin: {summary["min"]:.4f}\nMax: {summary["max"]:.4f}\nN: {n}\n10th: {metrics_diff[metrics_diff["resolution_diff"] == diff][metric].quantile(0.1):.4f}\n90th: {metrics_diff[metrics_diff["resolution_diff"] == diff][metric].quantile(0.9):.4f}', fontsize=8, transform=ax.transAxes)
        ...
        # set the title
        fig.suptitle(f'Metric by Resolution Difference: {diff.upper()}m')
...
def scatter_plot_covariates_and_metrics_by_resolution(metrics, plot_output_dir):
    ...
    resolutions = metrics['resolution_m']
    metrics.drop(columns='resolution_m', inplace=True)
    metrics['resolution_m'] = pd.Categorical(resolutions, categories=[10, 5, 3], ordered=True)
    ...
    for i, (covariate, metric) in enumerate(product(['median_slope_log', 'freq_high_dev_log'], metrics_list)):
        ...
        # make a regression plot
        sns.scatterplot(
            x=covariate, y=metric, data=metrics, ax=ax, hue='resolution_m', palette='bright', hue_order=[10, 5, 3]
        )
        ...
        for r, c in zip([10, 5, 3], ['blue', 'orange', 'green']):
            # fit regression line using lingress
            slope, intercept, r_value, p_value, std_err = stats.linregress(
                metrics.loc[metrics.resolution_m == r, covariate].values, metrics.loc[metrics.resolution_m == r, metric].values
            )
...
def metric_difference_by_covariate(metrics, plot_output_dir, hist_var='median_slope_log', diff='3-10', subset='all'):
    '''
    ...
    if hist_var == 'median_slope_log':
        ...
        x_axis_label = 'Log of Median Slope'
        title = f'Log(Median Slope) by Metric Diff. ({diff}m and {subset} scenarios)'
    else:
        x_var = 'freq_high_dev_log'
        x_axis_label = 'Log of Frequency High-Dev Land Cover'
        title = f'Log(Freq. High-Dev LC) by Metric Diff. ({diff}m and {subset} scenarios)'
    ...
    for i, metric in enumerate(metrics_list):
        ...
        # make a boxplot
        sns.histplot(
            x=x_var, data=metrics_diff, ax=ax, hue=f'{metric}_diff_positive', palette='bright', bins=250, kde=True
        )
    ...
    # save the figure
    plt.savefig(os.path.join(plot_output_dir, f'histogram_{hist_var}_by_metric_diff_{diff}_{subset}.png'))
...
if __name__ == '__main__':
        ...
        # set pandas rows and columns to display all
        pd.set_option('display.max_columns', None)
        pd.set_option('display.max_rows', None)

        data_dir = os.path.join(os.path.expanduser('~'), 'data', 'foss_fim', 'misc', 'resolution_analysis')

        metrics_fp = os.path.join(data_dir, 'metrics', 'skill', 'merged_skill_metrics_with_covariates.gpkg')
        metrics = gpd.read_file(metrics_fp)

        compute_metrics_gpkg = os.path.join(data_dir, 'metrics', 'compute','merged_compute_metrics_with_covariates.parquet')
        compute_metrics = pd.read_parquet(compute_metrics_gpkg)

        plot_output_dir = os.path.join(data_dir, 'plots', 'metric_analysis')
        ...
        ble_bool = metrics.scenario.str.startswith('ble')
        mip_bool = ~ble_bool
        metrics.loc[ble_bool, 'collection_id'] = 'ble'
        metrics.loc[mip_bool, 'collection_id'] = 'mip'

        # rename scenarios
        metrics['scenario'] = metrics.scenario.apply(lambda r: str(r).split('-')[1])

        # update categories
        metrics['resolution_m'] = metrics['resolution_m'].astype('category')
        metrics['scenario'] = metrics['scenario'].astype('category')
        metrics['collection_id'] = metrics['collection_id'].astype('category')

        # count unique stac_item_id by collection_id
        print(f"Number of unique stac_item_id by collection_id: {metrics.groupby('collection_id')['stac_item_id'].nunique()}")
        ...
        box_plots_by_resolution_and_magnitude(metrics[mip_bool], 'mip', plot_output_dir)
        box_plots_by_resolution_and_magnitude(metrics[ble_bool], 'ble', plot_output_dir)
        
        print('Computing scatter plots...')
        for benchmark_source in ['ble', 'mip']:
        ...
        #boxplot_of_algorithm_performance(metrics, 'mip', plot_output_dir)
        ...
        # HUCs completed by resolution for:  (10m),  (5m),  (3m), total
        box_plot_of_compute_performance_by_resolutions(compute_metrics, plot_output_dir, usd=False)
        box_plot_of_compute_performance_by_resolutions(compute_metrics, plot_output_dir, usd=True, device='s3')
        box_plot_of_compute_performance_by_resolutions(compute_metrics, plot_output_dir, usd=True, device='efs')
        ...
        for hist_var, diff, subset in tqdm(list(product(
            ['median_slope_log', 'freq_high_dev_log'],
            ['3-10', '3-5', '5-10'],
            ['all', '2yr', '5yr', '10yr', '25yr', '50yr', '100yr']
        ))):
            if subset == 'all':
                metric_difference_by_covariate(metrics, plot_output_dir, hist_var=hist_var, diff=diff, subset=subset)
            else:
                metric_difference_by_covariate(
                    metrics.query(f"scenario == '{subset}'"), plot_output_dir, hist_var=hist_var, diff=diff, subset=subset
                )
